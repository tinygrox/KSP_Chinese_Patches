...
public class KerbalismPatches : AbstractPatchBase
{
    ...
    #region Kerbalism 医疗舱 bug 修复补丁 - 官方已修复
    ...
    private static void ApplyKerbalismPatches()
    {
        ...
        UnityEngine.Debug.Log("[KSPCNPatches] ApplyKerbalismPatches Init!");
        ...
        GetHarmony.Patch(original: AccessTools.Method(AccessTools.TypeByName("KERBALISM.VesselRecovery_OnVesselRecovered"), "RecoverScienceData", new []{ AccessTools.TypeByName("KERBALISM.KsmScienceData"), typeof(ProtoPartModuleSnapshot), typeof(ProtoVessel), typeof(bool), typeof(double).MakeByRefType() }), transpiler: new HarmonyMethod(typeof(KerbalismPatches), nameof(KerbalismPatches.VesselRecovery_OnVesselRecovered_RecoverScienceDataPatch)));

        UnityEngine.Debug.Log("[KSPCNPatches] ApplyKerbalismPatches Done!");
    }
    // 我知道很硬，那没办法了，为了性能
